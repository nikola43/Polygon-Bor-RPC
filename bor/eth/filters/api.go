...
const (
	...
	maxTxHashes = 200
	// The maximum number of addresses allowed in an newPendingTransactionsWithFilter filter
	maxPendingTxFilterAddresses = 1000
)

// PendingTxFilterArgs represents the arguments for newPendingTransactionsWithFilter.
type PendingTxFilterArgs struct {
	FromAddress json.RawMessage `json:"fromAddress"`
	ToAddress   json.RawMessage `json:"toAddress"`
	HashesOnly  *bool           `json:"hashesOnly"`
}

// parseAddresses parses a json.RawMessage that may be a single hex address string
// or an array of hex address strings into a slice of common.Address.
func parseAddresses(raw json.RawMessage) ([]common.Address, error) {
	...
	if err := json.Unmarshal(raw, &single); err == nil {
		addr, err := decodeAddress(single)
		if err != nil {
			return nil, fmt.Errorf("invalid address %q: %v", single, err)
		}
	...
	if err := json.Unmarshal(raw, &arr); err != nil {
		return nil, fmt.Errorf("address must be a hex string or array of hex strings")
	}
	addrs := make([]common.Address, len(arr))
	for i, s := range arr {
...
// newPendingTransactionsWithFilter creates a subscription that fires for pending transactions
// matching the given filter criteria.
// If fromAddress/toAddress are specified, only transactions matching
// those addresses are sent. If hashesOnly is true, only transaction hashes are sent.
func (api *FilterAPI) NewPendingTransactionsWithFilter(ctx context.Context, args *PendingTxFilterArgs) (*rpc.Subscription, error) {
	notifier, supported := rpc.NotifierFromContext(ctx)
	if !supported {
		return &rpc.Subscription{}, rpc.ErrNotificationsUnsupported
	}
	...
	if args == nil {
		args = &PendingTxFilterArgs{}
	}

	fromAddrs, err := parseAddresses(args.FromAddress)
	if err != nil {
		return nil, fmt.Errorf("invalid fromAddress: %v", err)
	}

	toAddrs, err := parseAddresses(args.ToAddress)
	if err != nil {
		return nil, fmt.Errorf("invalid toAddress: %v", err)
	}

	if len(fromAddrs)+len(toAddrs) > maxPendingTxFilterAddresses {
		return nil, fmt.Errorf("too many filter addresses: %d (max %d)", len(fromAddrs)+len(toAddrs), maxPendingTxFilterAddresses)
	}

	hashesOnly := args.HashesOnly != nil && *args.HashesOnly

	// Build lookup sets for O(1) filtering
	fromSet := make(map[common.Address]struct{}, len(fromAddrs))
	for _, a := range fromAddrs {
		fromSet[a] = struct{}{}
	}

	toSet := make(map[common.Address]struct{}, len(toAddrs))
	for _, a := range toAddrs {
		toSet[a] = struct{}{}
	}

	hasFromFilter := len(fromSet) > 0
	hasToFilter := len(toSet) > 0
			...
			case txBatch := <-txs:
				...
				for _, tx := range txBatch {
					...
					if hasFromFilter || hasToFilter {
						...
						// Alchemy behavior: if both filters set, match from OR to
						if hasFromFilter && hasToFilter {
							if !matchFrom && !matchTo {
								continue
							}
						} else if hasFromFilter && !matchFrom {
							continue
						} else if hasToFilter && !matchTo {
							continue
						}
					}
